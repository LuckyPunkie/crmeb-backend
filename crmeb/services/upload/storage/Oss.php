...
//        }
    ...
    public function move(string $file = 'file', $realName = false)
    {
        ...
        try {
            ...
            $this->fileInfo->filePath = trim($this->cdn ?: $this->uploadUrl,'/') .'/'. ltrim($key,'/');
    ...
    public function stream($fileContent, string $key = null)
    {
        try {
            ...
            if (is_resource($fileContent)) {
                $fileContent = new Stream($fileContent);
            }
...
//                    if (systemConfig('image_thumbnail_status') && isset($config[$height]) && isset($config[$width])) {
                    ...
                    if (isset($config[$height]) && isset($config[$width])) {
                        if (!strpos($filePath, '?x-oss-process')) {
                            $this->fileInfo->$key = $filePath . '?x-oss-process=image/resize,w_' . $config[$width];
                        }
        ...
        if ($waterConfig['image_watermark_status'] && $filePath) {
            ...
            switch ($waterConfig['watermark_type']) {
                case 1://图片
                    ...
                    $waterPath = $filePath . '/watermark,image_' . base64_urlSafeEncode(substr(parse_url($waterConfig['watermark_image'],PHP_URL_PATH), 1)) . ',t_' . $waterConfig['watermark_opacity'] . ',g_' . ($this->position[$waterConfig['watermark_position']] ?? 'nw') . ',x_' . $waterConfig['watermark_x'] . ',y_' . $waterConfig['watermark_y'];
                    break;
                case 2://文字
