...
class Cos extends BaseUpload
{
    ...
    protected function upload(string $file = null, bool $isStream = false, string $fileContent = null)
    {
        ...
        try {
            ...
            return $this->setError($this->formatErrorMessage($e->getMessage()));
        } catch (\Throwable $e) {
            return $this->setError($this->formatErrorMessage($e->getMessage()));
        }
    }
...
//                    if (systemConfig('image_thumbnail_status', 1) && isset($config[$height]) && isset($config[$width])) {
    ...
    public function createBucket(string $name, string $region = '', string $acl = 'public-read')
    {
        ...
        try {
            ...
            if (strstr($e->getMessage(), '404') !== false) {
                return $this->setError('COS:' . $this->formatErrorMessage($e->getMessage()));
            }
        }
        //创建桶
        try {
            ...
            if (strstr($e->getMessage(), '[curl] 6') !== false) {
                return $this->setError('COS:无效的区域!!');
            } elseif (strstr($e->getMessage(), 'Access Denied.') !== false) {
    ...
    public function getDomian(string $name, string $region = null)
    {
        ...
        try {
            ...
            //$domainRules = $res['DomainRule'];
            //return [$domainRules['Name']];
    ...
    protected function setMessage(string $message)
    {
        $data = [
            ...
            'Due to your account is arrears' => '腾讯云账号已欠费，请充值后再使用 COS 服务。'
        ];
        $msg = $data[$message] ?? '';
        if ($msg) {
            return $msg;
    ...
    {
        return $this->setMessage($message) ?: $message;
    }
