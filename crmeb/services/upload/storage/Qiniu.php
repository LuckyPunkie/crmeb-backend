...
class Qiniu extends BaseUpload
{
    ...
    public function stream($fileContent, string $key = null)
    {
        ...
        try {
            ...
            $this->fileInfo->filePath = trim(($this->cdn ?: $this->uploadUrl),'/') . '/'.$key;
            $this->fileInfo->fileName = $key;
        ...
        if ($waterConfig['image_watermark_status'] && $filePath) {
            ...
            switch ($waterConfig['watermark_type']) {
                ...
                case 2://文字
                    ...
                    $fontSize = $waterConfig['watermark_text_size'] / 72 * 20 * 240;
                    $waterPath = $filePath .= '/2/text/' . base64_encode($waterConfig['watermark_text']) . '/fill/' . base64_encode($waterConfig['watermark_text_color']) . '/fontsize/' . floor($fontSize) . '/dissolve/' . $waterConfig['watermark_opacity'] . '/gravity/' . ($this->position[$waterConfig['watermark_position']] ?? 'SouthEest') . '/dx/' . $waterConfig['watermark_x'] . '/dy/' . $waterConfig['watermark_y'];
                    break;
            }
        }
        return $waterPath;
    ...
    public function createBucket(string $name, string $region = 'z0')
    {
        ...
        if (!$ret->ok()) {
            ...
            if ('bucket exists' === $error->message()) {
                return $this->setError('七牛云：云空间已存在，请尝试其他名称');
            }
