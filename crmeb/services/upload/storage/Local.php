...
class Local extends BaseUpload
{
     ...
     */
    public function move(string $file = 'file', $realName = false, $water = true)
    {
        ...
        return $this->afterUpload($this->isImageFileHandle($fileHandle), $this->fileInfo->filePath, $this->fileInfo->fileName);
    }
    ...
    public function stream($fileContent, string $key = null)
    {
        ...
        try {
            return $this->afterUpload($this->isImageByPath($key), $this->fileInfo->filePath, $this->fileInfo->fileName);
        } catch (\Throwable $e) {
    ...
    public function thumb(string $filePath = '', string $fileName = '', string $type = 'all')
    {
        ...
        //地址存在且不是远程地址
        $filePath = str_replace(systemConfig('site_url'), '', $filePath);
        if ($filePath && !$this->checkFilePathIsRemote($filePath) && file_exists(root_path().'public'.$filePath)) {
            $findPath = str_replace($fileName, $type . '_' . $fileName, $filePath);
            if (file_exists('.' . $findPath)) {
                return [
                    $type => str_replace($fileName, $type.'_' . $fileName, $filePath)
                ];
            }
            try {
                $this->water($filePath);
                foreach ($this->thumb as $v) {
                    if ($type == 'all' || $type == $v) {
                        ...
                        //防止重复生成
                        if (!file_exists('.' . $savePath) && isset($config[$width]) && $config[$height]) {
                        ...
                        $data[$v] = $this->fileInfo->$key = systemConfig('site_url').$savePath;
                    }
                }
            } catch (\Throwable $e) {
    ...
    public function image(string $filePath, array $waterConfig = [])
    {
        ...
        if ($watermark_image && $this->checkFilePathIsRemote($watermark_image)) {
            ...
            if ($pathName == $watermark_image) {//不再本地  继续下载
                ...
                $watermark_image = root_path().'public'. $this->defaultPath . '/' . $this->thumbWaterPath . '/' . $name;
                if (!file_exists($watermark_image)) {
                    try {
                        $down = app()->make(DownloadImageService::class);
                        $data = $down->downloadImage($waterConfig['watermark_image'], $this->thumbWaterPath,$name);
        ...
        if (!$watermark_image) {
            throw new AdminException('水印图片不存在');
        }
        $watermark_image =  root_path().'public'.$watermark_image;
    ...
    public function compress(string $filePath = '')
    {
        ...
        $filePath = str_replace(systemConfig('site_url'), '', $filePath);
        if (!$filePath || $this->checkFilePathIsRemote($filePath) || !$this->imageCompressStatus) {
            return $filePath;
        }

        $savePath = root_path() . 'public' . $filePath;
        if (!file_exists($savePath) || !$this->checkImage($savePath)) {
            return $filePath;
        }

        $quality = max(1, min(100, (int)$this->imageCompressQuality));
        try {
            Image::open($savePath)->save($savePath, null, $quality, true);
        } catch (\Throwable $e) {
            throw new AdminException($e->getMessage());
        }
        return $filePath;
