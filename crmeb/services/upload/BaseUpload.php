...
abstract class BaseUpload extends BaseStorage
{
     ...
     * @var string[]
     */
//    protected $thumb = ['thumb_big', 'thumb_mid', 'thumb_small','thumb_water_big', 'thumb_water_mid', 'thumb_water_small'];
    ...
    /**
     * 上传场景：auto 自动识别，image 图片，file 普通文件
     * @var string
     */
    protected $uploadScene = 'auto';

    /**
     * 是否开启图片压缩
     * @var bool
     */
    protected $imageCompressStatus = true;

    /**
     * 图片压缩质量
     * @var int
     */
    protected $imageCompressQuality = 80;
    ...
    protected function initialize(array $config)
    {
        ...
        $this->image_thumb_status = $config['image_thumb_status'];
        $this->waterConfig = array_merge($this->waterConfig, $config['water'] ?? []);
        $this->imageCompressStatus = (bool)Config::get($this->configFile . '.image_compress_status', true);
        $this->imageCompressQuality = (int)Config::get($this->configFile . '.image_compress_quality', 80);
    }
    ...
    {
        return [
            'filesize' => Config::get($this->configFile . '.filesize', []),
            'fileExt' => Config::get($this->configFile . '.iamge_fileExt', []),
            'fileMime' => Config::get($this->configFile . '.image_fileMime', []),
        ];
    }
     ...
     * @return $this
     */
    public function asImage(?array $validate = null)
    {
        $this->uploadScene = 'image';
        return $this->validate($validate ?? $this->getImageConfig());
    }
     ...
     * @return $this
     */
    public function asFile(?array $validate = null)
    {
        $this->uploadScene = 'file';
        return $this->validate($validate ?? $this->getConfig());
    }
     ...
     * @return bool|string
     */
    protected function validateFileHandle($fileHandle)
    {
        if (!$this->validate) {
            return true;
        }

        $extension = strtolower((string)$fileHandle->getOriginalExtension());
        $mime = strtolower((string)$fileHandle->getOriginalMime());
        $allowedExt = array_map('strtolower', $this->validate['fileExt'] ?? []);
        $allowedMime = array_map('strtolower', $this->validate['fileMime'] ?? []);
        $maxSize = (int)($this->validate['filesize'] ?? 0);
        $size = method_exists($fileHandle, 'getSize') ? (int)$fileHandle->getSize() : 0;

        if ($allowedExt && !in_array($extension, $allowedExt, true)) {
            return '不合法的文件后缀:' . $fileHandle->getOriginalExtension();
        }
        if ($maxSize > 0 && $size > $maxSize) {
            return '文件过大';
        }
        if ($allowedMime && !in_array($mime, $allowedMime, true)) {
            return '不合法的文件类型:' . $fileHandle->getOriginalMime();
        }
        return true;
    }
     ...
     * @return bool
     */
    protected function isImageFileHandle($fileHandle): bool
    {
        if (!$fileHandle) {
            return false;
        }
        if ($this->uploadScene === 'image') {
            return true;
        }
        if ($this->uploadScene === 'file') {
            return false;
        }

        $extension = strtolower((string)$fileHandle->getOriginalExtension());
        $mime = strtolower((string)$fileHandle->getOriginalMime());
        $imageExt = array_map('strtolower', Config::get($this->configFile . '.iamge_fileExt', []));
        $imageMime = array_map('strtolower', Config::get($this->configFile . '.image_fileMime', []));
        if ($extension && in_array($extension, $imageExt, true)) {
            return true;
        }
        if ($mime && in_array($mime, $imageMime, true)) {
            return true;
        }

        $realPath = $fileHandle->getRealPath();
        return $realPath ? $this->checkImage($realPath) : false;
    }
     ...
     * @return bool
     */
    protected function isImageByPath(string $path = ''): bool
    {
        if ($this->uploadScene === 'image') {
            return true;
        }
     ...
     * @return mixed
     */
    protected function afterUpload(bool $isImage, string $filePath = '', string $fileName = '')
    {
        $this->fileInfo->isImage = $isImage;
        $path = $filePath ?: ($this->fileInfo->filePath ?? '');
        if (!$isImage) {
            $this->fileInfo->filePathWater = $path;
            $this->fileInfo->filePathBig = '';
            $this->fileInfo->filePathMid = '';
            $this->fileInfo->filePathSmall = '';
            return $this->fileInfo;
        }

        $fileName = $fileName ?: ($this->fileInfo->fileName ?? '');
        if ($this->image_thumb_status && $this->authThumb) {
            $this->thumb($path, $fileName);
            if ($this->name !== 'local') {
                $this->fileInfo->filePathWater = $this->water($path);
            }
        } else {
            $this->fileInfo->filePathWater = $this->water($path);
            $this->fileInfo->filePathBig = $this->fileInfo->filePathMid = $this->fileInfo->filePathSmall = $this->fileInfo->filePathWater;
        }
     ...
     * @return mixed
     */
    abstract public function thumb(string $filePath = '', string $fileName = '', string $type = 'all');
