...
class SwooleTaskService
{
    ...
    {
        unset($workId);
        $payload = ['type' => $this->type, 'data' => $this->data];

        try {
            return Queue::push(SwooleTaskJob::class, $payload, env('SWOOLE_TASK_QUEUE', 'swoole_task'));
        } catch (\Throwable $e) {
            Log::info('发送 SwooleTask 队列失败' . $e->getMessage());
        }
