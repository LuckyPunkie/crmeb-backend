...
class ExcelService
{
    ...
    public  $paytype = [0 => '余额', 1 => '微信', 2 => '微信', 3 => '微信', 4 => '支付宝', 5 => '支付宝', 6 => '微信',7 => '线下支付',  8 => '微信', 9 => '支付宝'];
    ...
    public function order(array $where, int $page, int $limit)
    {
        ...
        foreach ($list as $item) {
            ...
            $one = [
                ...
                $item['remark'] ?: '',
                $item['is_del'] ? '是' : '否',
                json_encode($item['order_extend'] ?? '', JSON_UNESCAPED_UNICODE),
            ];
            $export[] = $one;
        }
        $header = ['商户名称', '订单编号', '订单状态', '订单类型', '推广人', '用户信息', '商品名称', '商品规格', '商品数量', '商品价格', '商品成本价' ,'优惠', '实付邮费(元)', '实付金额(元)', '已退款金额(元)', '收货人', '收货人电话', '收货地址', '物流/电话', '下单时间', '支付方式', '支付状态', '自提点/服务点','商家备注', '是否删除','系统表单信息'];
    ...
    public function pointsOrder(array $where, int $page, int $limit)
    {
        ...
        $paytype = [0 => '余额', 1 => '微信', 2 => '小程序', 3 => 'H5', 4 => '支付宝', 5 => '支付宝扫码', 6 => '微信扫码',7 => '线下支付',  8 => '微信扫码枪支付', 9 => '支付宝扫码枪支付'];
        $make = app()->make(StoreOrderRepository::class);
//        $status = $where['status'];
//        unset($where['status']);
        if ($where['status'] == -10) {
    ...
    public function financial(array $where, int $page, int $limit)
    {
        ...
        foreach ($list as $v) {
            ...
            $groupOrder = $orderInfo['groupOrder'] ?? [];

            $group_order_sn = $wx && $v['financial_type'] != 'svip'
                ? ($groupOrder['group_order_sn'] ?? '')
                : '';
            $paytype = $v['financial_type'] == 'order' && isset($orderInfo['pay_type'])
                ? ($this->paytype[$orderInfo['pay_type']] ?? '')
                : '';
            $transaction_id = $wx
        ...
        */
        ...
        if (!$where['is_mer']) {
            ...
            foreach ($list as $value) {
                ...
                $export[] = [
                    $merchant ? (($merchant['is_trader'] ?? 0) ? '自营' : '非自营') : '平台',
                    $merchantCategory['category_name'] ?? '平台',
                    $merchant['mer_name'] ?? '平台',
                    $orderInfo['group_order_sn'] ?? '无数据',
                    $value['order_sn'],
                    isset($orderInfo['pay_type']) ? ($this->paytype[$orderInfo['pay_type']] ?? '') : '',
            ...
            foreach ($list as $value) {
                ...
                $i++;
                $mer_name = $mer_name ?: ($merchant['mer_name'] ?? '');
            }
    ...
    public function refundOrder(array $where, int $page, int $limit)
    {
        ...
        foreach ($data as $datum) {
            ...
            foreach ($datum['refundProduct'] as $value) {
                $refundProduct = $value['product'] ?? [];
                $cartInfo = $refundProduct['cart_info'] ?? [];
                $productInfo = $cartInfo['product'] ?? [];
                $product .= '【' . ($productInfo['product_id'] ?? '') . '】' . ($productInfo['store_name'] ?? '') . '*' . ($value['refund_num'] ?? 0) . ($productInfo['unit_name'] ?? '') . PHP_EOL;
            }
            $export[] = [
