...
class Start
{
    ...
    public function show()
    {
        ...
        $this->displayItem('crmeb    version', get_code_version());

        //http配置
        $httpConf = \config("swoole.http");
        $this->displayItem('http host', $httpConf["host"] ?? '0.0.0.0');
        $this->displayItem('http port', $httpConf["port"] ?? 8324);
        $this->displayItem('http worker_num', $httpConf['worker_num'] ?? swoole_cpu_num());
        ...
        $this->displayItem('可在命令行执行，查看更多 ','php think cm_cli');
