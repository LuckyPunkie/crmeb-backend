...
class ServiceHandler
{
    ...
    public function service_chat(array $result)
    {
        ...
        $service = app()->make(StoreServiceRepository::class)->get($payload[0]);
        ...
        $this->switchServiceChat($uid, $frame->fd, $service->mer_id, $data['uid']);

        // T1.4: XSS 过滤顺序修复 — 先 strip_tags 剥离标签，避免 htmlspecialchars_decode 还原后绕过
        ...
        // T2.3: 同进程直接推送(优先) + Redis Queue fallback
    ...
    public function ack(array $result)
    {
        ...
        app()->make(StoreServiceLogRepository::class)
            ->updateDeliverStatus($msgId, $status);
