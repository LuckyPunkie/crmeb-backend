...
use crmeb\services\DirectPushService;
...
class UserHandler
{
    ...
    public function login(array $data)
    {
        ...
        if (!$token) return app('json')->message('err_tip', '登录过期1');
        ...
        try {
            ...
            return app('json')->message('err_tip', '登录过期2');
        }
        if ('user' != $payload->jti[1])
            return app('json')->message('err_tip', '登录过期3');
    ...
    {
        return app('json')->success(['uid' => $data['uid']]);
    }
    ...
    {
        $this->closeServiceChat($result['uid'], $result['frame']->fd);
    }
    ...
    public function chat_end(array $result)
    {
        ...
        $this->closeChat($result['uid'], $result['frame']->fd);
    }

    public function service_chat_start(array $result)
    {
        $this->switchServiceChat($result['uid'], $result['frame']->fd, $result['data']['mer_id'] ?? 0, $result['data']['uid']);
    }

    public function chat_start(array $result)
    {
        app()->make(StoreServiceUserRepository::class)->online($result['uid'], 1);
        $this->switchChat($result['uid'], $result['frame']->fd, $result['data']['mer_id'] ?? 0);
    }
    ...
    public function service_chat(array $result)
    {
        ...
        $service = app()->make(StoreServiceRepository::class)->getService($result['uid'], (int)$data['mer_id']);
        ...
        $this->switchServiceChat($result['uid'], $frame->fd, $service->mer_id, $data['uid']);

        // T1.4: XSS 过滤顺序修复 — 先 strip_tags 剥离标签
        $data['msn'] = trim(strip_tags($data['msn']));
        if (!$data['msn']) return app('json')->message('err_tip', '内容字符无效');
        ...
        // T2.3: 同进程直接推送优先
        $pushSuccess = DirectPushService::pushToUser([
            'uid'    => $data['uid'],
            'data'   => $log,
            'except' => [$frame->fd],
            'msg_id' => $log['service_log_id'],
        ]);
     ...
     * T2.1: ACK 消息确认
     */
    public function ack(array $result)
    {
        $msgId = $result['data']['msg_id'] ?? 0;
        $status = $result['data']['status'] ?? '';
        if (!$msgId) return app('json')->message('err_tip', '参数错误');
        app()->make(StoreServiceLogRepository::class)->updateDeliverStatus($msgId, $status);
    ...
    public function send_chat(array $result)
    {
        ...
        $service = app()->make(StoreServiceRepository::class)->getChatService($data['mer_id'], $result['uid']);
        if (!$service)
        ...
        // T1.5: 消息频率限制
        if (!RateLimiter::check($result['uid'], 30, 5)) {
            return app('json')->message('err_tip', '发送消息过于频繁，请稍后再试');
        }
        $this->switchChat($result['uid'], $frame->fd, $data['mer_id']);
        $data['uid'] = $result['uid'];
        $data['service_id'] = $service->service_id;
        $data['send_type'] = 0;
        $storeServiceLogRepository = app()->make(StoreServiceLogRepository::class);
        try {
            $storeServiceLogRepository->checkMsn($data['mer_id'], $result['uid'], $data['msn_type'], $data['msn']);
        } catch (ValidateException $e) {
            return app('json')->message('err_tip', $e->getMessage());
        }
        if ($data['msn_type'] == 100) {
