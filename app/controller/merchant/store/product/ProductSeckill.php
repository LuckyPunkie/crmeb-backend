...
class ProductSeckill extends BaseController
{
    ...
    public function lst()
    {
        ...
        $where = $this->request->params(['cate_id','keyword','mer_cate_id','seckill_status','us_status','mer_labels']);
        $type = $this->request->param('type',1);
        $where = array_merge($where,$this->repository->switchType($type,$this->request->merId(),1));
    ...
    public function updateSort($id)
    {
        // 获取请求参数中的排序值
        $sort = $this->request->param('sort');
        // 实例化 StoreSeckillActiveRepository 类
        app()->make(StoreSeckillActiveRepository::class);
        // 调用 repository 对象的 updateSort 方法更新商品排序
        $this->repository->updateSort($id, $this->request->merId(), ['sort' => $sort]);
        // 返回 JSON 格式的修改成功提示
        return app('json')->success('修改成功');
    }
    ...
    /**
     * 预览商品信息
     * @return \think\response\Json 返回 JSON 格式的商品信息
     */
    public function preview()
    {
        ...
        $data['mer_status'] = 1;
        $data['rate'] = 3;
        // 调用 repository 对象的 preview 方法获取商品信息并返回 JSON 格式的结果
