...
class Auth extends BaseController
{
    ...
    public function login(UserRepository $repository)
    {
        ...
        $pid = $this->request->param('spread/d', 0);
    ...
    public function mpAuth()
    {
        ...
        try {
            //解密获取用户信息
            $userInfo = MiniProgram::decryptData($userInfoCong['session_key'], $data['iv'], $data['encryptedData']);
        } catch (Exception $e) {
    ...
    private function authInfo($auth, $createUser = false)
    {
        ...
        if ($auth['type'] == 'routine') {
            $userInfoConfig = $make->getUserInfoByCode($code);
            $userInfo = $make->encryptorUserInfo($userInfoConfig, $data['iv'], $data['encryptedData']);
            $userInfo = array_merge($userInfoConfig, $userInfo);
            $user = $make->syncRoutineUser($userInfoConfig['openid'] ?? '', $userInfo, $createUser);
            if (!$user)
                throw new ValidateException('授权失败[001]');
            return $user;
        } else if ($auth['type'] == 'wechat') {
            $wechatInfo = $make->wechantAuth($code);
    ...
    {
        $auth = $this->request->param('auth/a');
    ...
    public function appAuth()
    {
        ...
        if (!empty($data['authResult']['access_token']) && !empty($data['authResult']['openid'])) {
            try {
                ...
                return app('json')->fail('微信授权校验失败');
            }
            $unionKey = (string)($raw['unionid'] ?? '');
            if ($unionKey === '') {
                return app('json')->fail('未返回 unionid，请将微信应用绑定开放平台后重试');
            }
            $userInfo = [
                'nickName' => $raw['nickname'] ?? '',
                'gender' => (int)($raw['sex'] ?? 0),
                'city' => $raw['city'] ?? '',
                'province' => $raw['province'] ?? '',
                'country' => $raw['country'] ?? '',
                'avatarUrl' => $raw['headimgurl'] ?? '',
                'openId' => $raw['openid'] ?? '',
            ];
            $user = $wechatUserRepository->syncAppUser($unionKey, $userInfo);
        } else {
            if (empty($data['userInfo']['unionId'])) {
                return app('json')->fail('授权失败');
            }
            $user = $wechatUserRepository->syncAppUser($data['userInfo']['unionId'], $data['userInfo']);
        }
        if (!$user)
            return app('json')->fail('授权失败');
        /** @var UserRepository $make */
    ...
    {
        $captchaType = $this->request->param('captchaType/s', 'clickWord');
    ...
    public function ajcheck()
    {
        ...
        $pointJson = $this->request->param('pointJson/s', '');
        $captchaType = $this->request->param('captchaType/s', 'clickWord');
        try {
