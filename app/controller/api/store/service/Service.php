...
class Service extends BaseController
{
    ...
    {
        $serviceId = (int)$this->request->param('service_id/d');
     ...
     * 小程序 WS 临时 Token 获取
     */
    public function tempWsToken()
    {
        $token = bin2hex(random_bytes(16));
        $userToken = $this->request->header('Authori-zation');
        if (!$userToken) {
            $userToken = $this->request->header('Authorization');
        }
        if (!$userToken) {
            return app('json')->fail('缺少认证信息');
        }
        Cache::set("ws_temp_token:{$token}", $userToken, 30);
