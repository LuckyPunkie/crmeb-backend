...
class Config extends BaseController
{
    ...
    {
        $cid = $this->request->param('config_classify_id',0);
        $form = $this->repository->form($cid);
    ...
    public function create()
    {
        ...
        if ($data['linked_status'] && !$data['linked_id']) return app('json')->fail('请选择关联显示上级');
    ...
    public function form($key, ConfigClassifyRepository $configClassifyRepository)
    {
        ...
        if ($configClassifyRepository->existsWhere(['pid' => $configClassfiy->config_classify_id]))
            $form = $this->repository->tabForm($configClassfiy, $this->request->merId(),$tab_id);
        else
    ...
    public function upload($field)
    {
        ...
        $data = $upload->to($merid.'/attach/')->validate()->move($field);
        if ($data === false) {
    ...
    {
        $data = $this->repository->getConfigKey($key);
        return app('json')->success($data);
    }

    public function getParams()
    {
        $data = $this->request->params(['user_type', 'config_classify_id', 'config_name', 'config_props', 'config_key', 'config_type', 'config_rule', 'required', 'info', 'sort', 'status','linked_status']);
        app()->make(ConfigValidate::class)->check($data);
        ...
        if (!$configClassifyRepository->exists($data['config_classify_id']))
            throw new ValidateException('配置分类不已存在');
        if ($data['linked_status']){
            $linked_data = $this->request->param('linked_data',[]);
            if (!$linked_data) throw new ValidateException('请选择关联显示上级');
            $data['linked_id'] = $linked_data[0];
            $data['linked_value'] = $linked_data[1];
        }
        return $data;
    }
}
