...
class ServiceTabConfig extends BaseController
{
    ...
    {
        return app('json')->success(['list' => $this->repository->listAll()]);
    }
    ...
    public function save()
    {
        ...
        if (!is_array($data['brands'])) $data['brands'] = [];
        $row = $this->repository->saveConfig($data);
    ...
    {
        $id = (int)$this->request->param('id', 0);
        $this->repository->deleteConfig($id);
