...
if (!function_exists('getMonth')) {
     ...
     * @param int $offset 0:当前时间，-1:上一个季度，1:下一个季度
     * @param $ceil
     * @return array
     */
    function getMonth($time = null, $offset = 0)
    {
        // 处理时间参数
        if ($time === null) {
            $timestamp = time();
        ...
        if ($targetQuarter > 4) {
            $targetQuarter -= 4;
            $targetYear += 1;
        } elseif ($targetQuarter < 1) {
            $targetQuarter += 4;
            $targetYear -= 1;
        }
        // 计算季度的首月和末月
        $firstMonth = ($targetQuarter - 1) * 3 + 1;
        $lastMonth = $targetQuarter * 3;

        // 生成季度的首日和末日
        $firstDay = date('Y-m-01', mktime(0, 0, 0, $firstMonth, 1, $targetYear));
        $lastDay = date('Y-m-t', mktime(0, 0, 0, $lastMonth, 1, $targetYear));
    ...
    function getModelTime(BaseQuery $model, string $section, $prefix = 'create_time', $field = '-',$time = '')
    {
        ...
        switch ($section) {
            ...
            case 'quarter':
                ...
                break;
            case 'lately7':
                $model = $model->where($prefix, 'between', [date('Y-m-d H:i:s', strtotime("-7 day")), date('Y-m-d H:i:s')]);
                break;
            case 'lately30':
                $model = $model->where($prefix, 'between', [date('Y-m-d H:i:s', strtotime("-30 day")), date('Y-m-d H:i:s')]);
                break;
            default:
 ...
 * @author Qinii
 */
if (!function_exists('momTime')) {
    function momTime(BaseQuery $model, string $section, $prefix = 'create_time', $field = '-')
    {
        switch ($section) {
            case 'today':
                $model->whereDay($prefix, 'yesterday');
                break;
            case 'week':
                $model->whereWeek($prefix, 'last week');
                break;
            case 'month':
                $model->whereMonth($prefix, 'last month');
                break;
            case 'year':
                $model->whereYear($prefix, 'last year');
                break;
            case 'yesterday':
            ...
            case 'quarter':
                //获取上季度开始时间
                list($startTime, $endTime) = getMonth($section, -1);
                $model = $model->whereBetweenTime($prefix, $startTime , $endTime);
                break;
            case 'lately7':
                $model = $model->whereBetweenTime($prefix, date('Y-m-d H:i:s', strtotime("-14 day")), date('Y-m-d H:i:s',
                    strtotime("-7 day")));
                break;
            case 'lately30':
                $model = $model->whereBetweenTime($prefix,date('Y-m-d H:i:s', strtotime("-60 day")), date('Y-m-d H:i:s',
                    strtotime("-30 day")));
                break;
            default:
                // 单日 → 上周同期
                ...
                if (strstr($section, $field) !== false) {
                    list($startTime, $endTime) = explode($field, $section);
                    // 计算时间跨度（天数）
                    $daysDiff = (strtotime($endTime) - strtotime($startTime)) / 86400 + 1;
                    // 根据时间跨度选择环比周期
                    if ($daysDiff == 1) {
                        // 单日：按周环比（上周同一天）
                        $interval = '-1 week';
                    ...
                    $compareStart = date('Y-m-d H:i:s', strtotime($startTime . $interval));
                    $compareEnd = date('Y-m-d H:i:s', strtotime($endTime . $interval));

                    $model->whereBetweenTime($prefix, $compareStart, $compareEnd);
                }
                break;
        }
    }
...
if (!function_exists('merchantConfig')) {
    ...
    {
        return app()->make(ConfigValueRepository::class)->getConfig(name: $key, merId: $merId);
    }
}
...
if (!function_exists('get_code_version')) {
    /**
     * 获取系统类型
     * @return string
     */
...
if (!function_exists('get_code_type')) {
    /**
     * 获取系统版类型
...
if (!function_exists('update_crmeb_compiled')) {
    ...
    function update_crmeb_compiled($default = 'v1.0.0')
    {
        ...
        $extractPath = $compiledPath . '/_runtime_' . $key;
        ...
        $mergeDir = function (string $from, string $to) {
            ...
            foreach ($iterator as $item) {
                ...
                if ($relativePath === '' || strpos($relativePath, '..' . DIRECTORY_SEPARATOR) !== false || strpos($relativePath, DIRECTORY_SEPARATOR . '..') !== false) {
                    continue;
                }
        ...
        try {
            if (is_file($file)) {
                ...
                if (!is_dir($extractPath) && !mkdir($extractPath, 0777, true) && !is_dir($extractPath)) {
                    return false;
                }
                $zip = new ZipArchive();
                ...
                if (is_dir($extractPath) && !$mergeDir($extractPath, $root)) {
                    $removeDir($extractPath);
                    return false;
                }
                $removeDir($extractPath);
...
if (!function_exists('lbs_address')) {
    function lbs_address($region, $address, $isError = false)
    {
        ...
        if ($res->status) {
            ...
            Log::info('地图key错误信息:'.$res->message);
            return [];
        }
        if (!$res->result) {
...
if (!function_exists('growthRate')) {
     ...
     * @author Qinii
     */
    function growthRate($current, $previous, $scale = 2)
    {
        // 转为字符串，避免浮点数精度问题
        $current = (string)$current;
        $previous = (string)$previous;

        if (bccomp($previous, '0', $scale) === 0) {
            // 上期为0的情况处理
            return bccomp($current, '0', $scale) === 1 ? '100.00' : '0.00';
        }

        // 计算增长额
        $growth = bcsub($current, $previous, $scale + 2);

        // 计算增长率
        $rate = bcdiv($growth, $previous, $scale + 4);
        $percentage = bcmul($rate, '100', $scale);
        return $percentage;
    }
}

if (!function_exists('getStepLength')) {
    function getStepLength($date, $format = null)
    {
        // 设置默认格式
        if ($format === null) {
            $format = [
        ...
        switch ($date) {
            ...
            case 'month':
                ...
                for ($day = 1; $day <= $days; $day++) {
                    $timestamp = strtotime(date('Y-m-') . sprintf('%02d', $day));
                    $dates[] = date(sprintf('%02d', $day), $timestamp);
                }
                break;
            ...
            case 'year':
                ...
                for ($month = 1; $month <= $currentMonth; $month++) {
                    $timestamp = strtotime(date('Y-') . sprintf('%02d', $month) . '-01');
                    $dates[] = date(sprintf('%02d', $month) , $timestamp);
                }
                break;
            ...
            case 'quarter':
                ...
                $endMonth = min($startMonth + 2, $currentMonth);
            ...
            default:
                ...
                if (count($customDate) !== 2) {
                    ...
                    // 自定义日期范围，例如：2023/12/01-2024-12-07,如果区间小于30天则按天返回;大于小于一年则按月返回;大于一年则按年返回
                    $startDate = strtotime($customDate[0]);
                    $endDate   = strtotime($customDate[1]);
                    $diffDays = abs(($endDate - $startDate) / 86400); // 计算天数差
                    if ($diffDays <= 30) {
                        // 按天返回
                        while ($startDate <= $endDate) {
                            $dates[] = date($format, $startDate);
                            $startDate += 86400; // 加一天的时间戳
                        }
                        $customFormat = '%m-%d';
                    } elseif (date('Y', $startDate) === date('Y', $endDate)) {
                        // 按月返回
                        for ($month = date('m', $startDate); $month <= date('m', $endDate); $month++) {
                            $timestamp = strtotime(date('Y-') . sprintf('%02d', $month) . '-01');
                            $dates[] = date(sprintf('%02d', $month) , $timestamp);
                        }
                        $customFormat = '%m';
...
if (!function_exists('getWhereParams')) {
    ...
    /**
     * 安全获取 $where 参数（支持类型验证 + 默认值 + 零值有效）
     * @param array  $where   请求参数数组
     * @param string $key     键名
     * @param mixed  $default 默认值（默认 null）
     * @param string $type    验证类型：string|int|float|bool|array|numeric|json|in:1,2,3
     * @return mixed
     */
    function getWhereParams($where, $key, $default = '', $type = 'string')
    {
        if (!isset($where[$key])) {
            return $default;
        }
        ...
        switch ($type) {
            ...
            case 'array':
                return is_array($value) ? $value : (is_string($value) && $value !== '' ? explode(',', $value) : $default);
            ...
            default:
                // 支持 in:1,2,3 格式
                if (strpos($type, 'in:') === 0) {
                    $allow = explode(',', substr($type, 3));
                    $allow = array_map('trim', $allow);
                    return in_array($value, $allow, true) ? $value : $default;
                }
                return $value; // 默认返回原值
        }
    }
