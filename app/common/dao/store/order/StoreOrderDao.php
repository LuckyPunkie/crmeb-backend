...
class StoreOrderDao extends BaseDao
{
    ...
    public function whereSearch($query, $where, $sysDel = 0)
    {
        ...
        $query->when(isset($where['activity_type']) && $where['activity_type'] !== '', function ($query) use ($where) {
            ...
            ->when(isset($where['status']) && $where['status'] !== '', function ($query) use ($where) {
                switch ($where['status']) {
                    case 0 :
                        $query->whereIn('StoreOrder.status', [0, 9, 20]);
                        break;
                    case -2 :
            ...
            ->when(isset($where['is_verify']) && $where['is_verify'], function ($query) use ($where) {
                ...
                $query->where(function($query) use($where){
                    $query->where(function ($query) {
                        ...
                        $query->where('StoreOrder.order_type', 2)->where('StoreOrder.status', 0);
                    })->whereOr(function ($query) {
                        $query->where('StoreOrder.order_type', 2)->where('StoreOrder.status', 1)->where('enable_assigned', 1);
                    });
                });
            })
            ...
            ->when(isset($where['mom_date']) && $where['mom_date'] !== '', function ($query) use ($where) {
                momTime($query, $where['mom_date'], 'StoreOrder.create_time');
            })
            ...
            ->when(isset($where['group_order_sn']) && $where['group_order_sn'] !== '', function ($query) use ($where) {
                $query->join('StoreGroupOrder GO', 'StoreOrder.group_order_id = GO.group_order_id')
                    ->where('GO.group_order_sn', $where['group_order_sn']);
            })
            ->when(isset($where['merchant_take_id']) && $where['merchant_take_id'] !== '', function ($query) use ($where) {
                $query->where('merchant_take_id', $where['merchant_take_id']);
            })
            ...
            ->when(isset($where['delivery_keywords']) && $where['delivery_keywords'] !== '', function ($query) use ($where) {
                $query->where(function ($query) use ($where) {
                    $query->whereLike('StoreOrder.order_sn', "%" . $where['delivery_keywords'] . "%");
                    $orderId = StoreOrderProduct::alias('op')
                        ->join('storeProduct sp', 'op.product_id = sp.product_id')
                        ->whereLike('store_name', "%{$where['delivery_keywords']}%")->column('op.order_id');
                    if($orderId) {
                        $query->whereOr(function ($query) use ($orderId) {
                            if($orderId) {
                                $query->whereIn('StoreOrder.order_id', $orderId);
                            }
                        });
                        ...
                        switch ($reservationStatus) {
                            case 1: // 待服务
                                $query->whereOr(function ($query) {
                                    $query->whereIn('StoreOrder.status', '0,1');
                                });
                                break;
                            case 2: // 服务中 只有上门
                                $query->whereOr(function ($query) {
                                    $query->where('StoreOrder.status', '20')->where('order_type', 0);
                                });
                                break;
                            case 3: // 已完成
                                $query->whereOr(function ($query) {
                                    $query->whereIn('StoreOrder.status', '2,3');
                                });
                                break;
                            default:
                                $query->whereOr(function ($query) {
                                    $query->where(function ($query) { // 上门
                                        $query->where('StoreOrder.status', '1')->where('order_type', 0);
                                    });
                                    $query->whereOr(function ($query) { // 到店
                                        $query->whereIn('StoreOrder.status', '0,1')->where('order_type', 1);
                                    });
                                });
                                break;
                        }
    ...
    public function searchAll(array $where, $sysDel = 0, $is_points = null)
    {
        ...
        // 分组权限
        if (app('request')->hasMacro('regionAuthority') && $region = app('request')->regionAuthority()) {
            $query->whereIn('StoreOrder.mer_id', $region);
        }
        $query->when(isset($where['activity_type']) && $where['activity_type'] !== '', function ($query) use ($where) {
            ...
            ->when(isset($where['status']) && $where['status'] !== '', function ($query) use ($where) {
                switch ($where['status']) {
                    case 0 :
                        ...
                        break;
                    case -2 :
                        $query->whereNotIn('StoreOrder.status', [10, 11])->where('is_del', 0);
                        break;
                    case -3 :
    ...
    {
        return StoreOrder::getDB()->with(['deliveryOrder'])->where('order_id', $id)->where('uid', $uid)->where('is_del', 0)->where('paid', 1)->where('is_system_del', 0)->find();
    }
    ...
    {
        $query = StoreOrder::getDB()->where('paid', 1)->when($merId, function ($query, $merId) {
            $query->where('mer_id', $merId);
        });
        // 分组权限
        if (app('request')->hasMacro('regionAuthority') && $region = app('request')->regionAuthority()) {
        ...
        return getModelTime($query, $day, 'pay_time')->sum('pay_price');
    }
    ...
    public function joinGroupOrder($where)
    {
        $query = StoreOrder::hasWhere('groupOrder',function($query) use($where){
            $query->when(isset($where['is_first']) && $where['is_first'] == 1, function($query) use($where){
