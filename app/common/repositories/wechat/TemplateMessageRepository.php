...
class TemplateMessageRepository extends BaseRepository
{
    ...
    public function form(?int $id = null,$type = 0)
    {
        ...
        $form->setRule([
            ...
            Elm::switches('status','状态：',1)->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启'),
        ]);
    ...
    public function updateForm($id)
    {
        ...
        $form->setRule([
            ...
            Elm::switches('status','状态：',$tem['status'])->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启'),
        ]);
            ...
            foreach ($all['list'] as $k => $template) {
                if ($template['tempkey']) {
                    ...
                    try {
                        ...
                        $errData[$k][] = '获取关键词列表失败：' . $wechatErr->getMessage(). '，微信错误代码：' . $e->getCode();
                    }
                    $kid = [];
                    if ($works) {
                    ...
                    if ($kid && isset($template['kid']) && !$template['kid']) {
                        $tempid = '';
                        try {
                            $tempid = MiniProgram::addSubscribeTemplate($template['tempkey'], $kid, $template['notice_title']);
                        } catch (\Throwable $e) {
                            $wechatErr = $e->getMessage();
                            ...
                            $errData[$k][] = '模板ID：'.$template['tempkey'].'，添加订阅消息模版失败：'.$wechatErr->getMessage().'，微信错误代码：'.$wechatErr->getCode();
                        }
    ...
    public function syncWechatSubscribe()
    {
        ...
        foreach ($tempall['list'] as $temp) {
            ...
            //添加模版消息
            try{
                $res = OfficialAccount::addTemplateId($temp['tempkey'],$name);
            }catch (\Exception $exception) {
                return $temp['notice_title'].$exception->getMessage();
            }

            if (($res['errcode'] ?? 0) == 0 && !empty($res['template_id'])) {
