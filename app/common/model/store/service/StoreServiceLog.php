...
class StoreServiceLog extends BaseModel
{
    ...
    {
        return $this->hasOne(ProductPresell::class, 'product_presell_id', 'msn')->append(['product'], true);
    }

    public function productGroup()
    {
        return $this->hasOne(ProductGroup::class, 'product_group_id', 'msn')->append(['product'], true);
    }
    ...
    {
        return $this->hasOne(User::class, 'uid', 'uid')->field('uid,avatar,nickname,mark');
    }
    ...
    public function getMsnInfoAttr($value)
    {
        $value = $this->msn;
        //4=商品 5=订单 6=退款单
        switch ($this->msn_type) {
            case 4:
                $mode = Product::where(['product_id' => $value])->find();
                $msn = '[商品ID:' .$value.'],'. $mode->store_name;
                break;
            case 5:
                $mode = StoreOrder::where(['order_id' => $value])->find();
                $msn = '[订单ID:' .$value.'],'. $mode->order_sn;
                break;
            case 6:
                $mode = StoreRefundOrder::where(['refund_order_id' => $value])->find();
                $msn = '[退款单ID:' .$value.'],'. $mode->refund_order_sn;
                break;
            default:
                $msn = $value;
                break;
